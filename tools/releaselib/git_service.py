...
class GitService:
    ...
    def add(self, file_path: Path):
        ...
        return self.run(
            ["git", "add", file_path]
        )  # file_path will be converted to string by _run_raw
    ...
    def assert_clean_index(self):
        ...
        try:
            ...
            if result.returncode == 1:
                ...
                raise GitServiceError(
                    f"Git command failed with exit code {result.returncode}: git diff-index --quiet HEAD --\n{result.stderr.decode('utf-8', errors='replace')}"
                )

        except FileNotFoundError as e:
            raise GitServiceError(
                "Git command not found. Is Git installed and in your PATH?", cause=e
            )
        except subprocess.TimeoutExpired as e:
            raise GitServiceError(
                f"Git command timed out after {self.timeout} seconds: git diff-index --quiet HEAD --",
                cause=e,
            )
