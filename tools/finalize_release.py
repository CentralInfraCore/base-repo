...
COLOR_CODES = {
    "DEBUG": "\033[90m", "INFO": "\033[0m", "WARNING": "\033[93m",
    "ERROR": "\033[91m", "CRITICAL": "\033[91m", "SUCCESS": "\033[92m",
}
RESET_CODE = "\033[0m"
...
def main():
    ...
    parser.add_argument("filepath", type=Path, help="Path to the project.yaml file to finalize.")
    parser.add_argument("--cic-vault-key", required=True, help="Name of the CIC Vault key for signing (e.g., 'cic-root-ca-key').")
    
    cert_group = parser.add_mutually_exclusive_group(required=True)
    cert_group.add_argument("--cic-cert-file", type=Path, help="Path to the file containing the CIC CA certificate to embed.")
    cert_group.add_argument("--cic-cert-vault-path", help="Vault path to the certificate, e.g., 'kv/data/secrets/my-cert:cert_key'.")

    parser.add_argument("--dry-run", action="store_true", help="Perform a trial run without writing the file.")
    ...
    try:
        ...
        if not args.filepath.exists():
            raise FileNotFoundError(f"The specified file was not found: {args.filepath}")
        if args.cic_cert_file and not args.cic_cert_file.exists():
            raise FileNotFoundError(f"The CIC certificate file was not found: {args.cic_cert_file}")
        ...
        if not vault_addr or not vault_token:
            raise VaultServiceError("VAULT_ADDR and VAULT_TOKEN environment variables must be set.")
        ...
        if not checksum or not build_hash:
            raise ValueError("'checksum' and/or 'buildHash' fields are missing from metadata.")
        if checksum != build_hash:
            raise ValueError("Validation failed: 'checksum' and 'buildHash' do not match!")
        
        logger.info("✓ Validation successful: checksum matches buildHash.")

        # 2. Embed CIC Certificate
        if args.cic_cert_vault_path:
            logger.info(f"Fetching CIC certificate from Vault path: {args.cic_cert_vault_path}...")
            try:
                path_parts = args.cic_cert_vault_path.split(':')
                if len(path_parts) != 2:
                    raise ValueError("Vault path must be in the format 'mount/path/to/secret:key'")
                
                full_secret_path, secret_key = path_parts
                mount_path, secret_name = full_secret_path.split('/data/', 1)
                mount_path += '/data'

                cic_certificate = vault_service.get_certificate(mount_path, secret_name, secret_key)
            except (ValueError, VaultServiceError) as e:
                raise VaultServiceError(f"Could not retrieve certificate from Vault: {e}") from e
        else:
        ...
        logger.info(f"Requesting signature from Vault with key '{args.cic_vault_key}'...")
        ...
        if args.dry_run:
            ...
            print(yaml.dump(project_data, sort_keys=False, indent=2, allow_unicode=True))
