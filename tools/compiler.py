...
class ColoredFormatter(logging.Formatter):
    def format(self, record):
        ...
        if level_name == "INFO" and "DRY-RUN" in log_message:
            ...
            level_name == "INFO" and "✓" in log_message
        ):  # Simple heuristic for success messages
            level_name = "SUCCESS"
...
def load_project_config():
    ...
    try:
        ...
        logger.critical(
            f"[FATAL] Could not load or parse compiler settings from project.yaml: {e}"
        )
        sys.exit(1)
...
def main():
    ...
    parent_parser.add_argument(
        ...
        type=int,
        default=60,
        help="Timeout for Git commands in seconds.",
    )
    parent_parser.add_argument(
        "--vault-timeout",
        type=int,
        default=10,
        help="Timeout for Vault API calls in seconds.",
    )
    parent_parser.add_argument(
    ...
    subparsers.add_parser(
        "validate",
        help="Validate all schemas without building or signing.",
    ...
    release_parser = subparsers.add_parser(
        "release",
        help="Build, checksum, and sign all non-dev schemas (requires Vault).",
        parents=[parent_parser],
    )
    ...
    try:
        ...
        if args.command == "validate":
            ...
            component_name, original_base_branch = manager.run_release_check(
            ...
            if args.dry_run:
                logger.info(
                    "[DRY-RUN] Release process simulation complete. Vault signing was simulated."
                )
            else:
                logger.info(
                    "✓ Release closed successfully. project.yaml has been finalized."
                )
                ...
                logger.info(
                    f"ACTION REQUIRED: Please push the changes and the tag: git push origin {release_branch_name} && git push origin {component_name}@v{release_version}"
                )
