...
def run_release():
    ...
    for schema_file in schema_files:
        ...
        metadata_for_signing['build_timestamp'] = datetime.now(timezone.utc).isoformat()
        metadata_for_signing['checksum'] = checksum # Add the just-calculated checksum
        
        # 3. Get signature from Vault
        digest_to_sign_b64 = get_sha256_b64(to_canonical_json(metadata_for_signing))
        print(f"  - Requesting signature from Vault...")
        try:
            response = requests.post(
                f"{vault_addr}/v1/transit/sign/{VAULT_KEY_NAME}",
                headers={"X-Vault-Token": vault_token},
                json={"input": digest_to_sign_b64, "prehashed": True, "hash_algorithm": "sha2-256"},
                verify=verify_tls
            )
            response.raise_for_status()
            signature = response.json()['data']['signature']
            print(f"  - Signature received successfully.")
        ...
        final_schema['metadata']['build_timestamp'] = metadata_for_signing['build_timestamp']
        ...
        try:
            ...
            print(f"  - \033[92m✓ Final validation passed.\033[0m")
        except Exception as e:
