...
def _parse_imports(source: str) -> dict[str, str]:
    ...
    for single_m in re.finditer(
        r'^\s*import\s+(?:(\w+)\s+)?"([^"]+)"', no_comments, re.MULTILINE
    ):
        alias = single_m.group(1) or single_m.group(2).split("/")[-1]
        imports[alias] = single_m.group(2)
    ...
    if block_m:
        for line in block_m.group(1).splitlines():
            ...
            if not line:
                continue
            # alias "path"
            aliased_m = re.match(r'(\w+|_|\.)\s+"([^"]+)"', line)
            if aliased_m:
                alias = (
                    aliased_m.group(1)
                    if aliased_m.group(1) not in ("_", ".")
                    else aliased_m.group(2).split("/")[-1]
                )
                imports[alias] = aliased_m.group(2)
                continue
            # "path"
            bare_m = re.match(r'"([^"]+)"', line)
            if bare_m:
                alias = bare_m.group(1).split("/")[-1]
                imports[alias] = bare_m.group(1)
...
def _extract_local_typed_vars(
    text: str, imports: dict[str, str], module_name: str = ""
) -> dict[str, str]:
...
def _suggest_tags(
    go_file: Path, imports: dict[str, str], objects: list[dict]
) -> list[str]:
...
def _parse_objects(
    source: str, imports: dict[str, str], module_name: str = ""
) -> list[dict]:
    objects: list[dict] = []
    clean = _clean(source)
    ...
    for m in re.finditer(
        r"\btype\s+(\w+)\s+(?!struct\b|interface\b)(\S+)", clean, re.MULTILINE
    ):
        name = m.group(1)
...
def main() -> None:
    ...
    parser.add_argument(
        "files", nargs="*", metavar="FILE", help="Go source files to process"
    )
    parser.add_argument("--dir", metavar="DIR", help="Process all .go files in DIR")
    parser.add_argument(
        "--recursive",
        action="store_true",
    ...
    parser.add_argument("--skip-tests", action="store_true", help="Skip _test.go files")
    parser.add_argument(
    ...
    for go_file in sorted(set(files)):
        ...
        if args.dry_run:
            print(
                f"DRY-RUN ({action}): {yaml_file}  [{obj_count} objects, {ref_count} refs]"
            )
        else:
