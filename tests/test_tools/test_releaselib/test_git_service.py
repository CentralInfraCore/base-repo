...
def test_run_called_process_error(mock_run, git_service):
    ...
    mock_run.side_effect = subprocess.CalledProcessError(1, 'git', stderr=b'error message')
    with pytest.raises(GitServiceError, match="Git command failed: git status\nerror message"):
...
def test_run_file_not_found_error(mock_run, git_service):
    ...
    with pytest.raises(GitServiceError, match="Git command not found: git status. Is Git installed and in your PATH?"):
        git_service.run(['git', 'status'])
...
def test_run_timeout_expired(mock_run, git_service):
    ...
    with pytest.raises(GitServiceError, match="Git command timed out after 30 seconds: git status"):
        git_service.run(['git', 'status'])
...
def test_archive_tree_bytes(mock_run_raw, git_service):
    ...
    mock_run_raw.assert_called_once_with(['git', 'archive', '--format=tar', '--prefix=./', 'some_tree_id'])
...
def test_merge_with_message(mock_run, git_service):
    ...
    mock_run.assert_called_once_with(['git', 'merge', '-m', 'Merge message', 'feature-branch'])
